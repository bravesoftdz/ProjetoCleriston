...
public class ProfessorBean {
	...
	public String cadastrarProfessor() {
		if (getProfessor().getNome().length() < 3) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Infome um nome com mais de 3 letras", ""));
	...
	public List<SelectItem> getSelectListProfessores() {
		...
		for (Professor professor : listaProfessores) {
			list.add(new SelectItem(professor, professor.getNome()));
		}
		return list;
	}

	public void setSelectListProfessores(List<Professor> listaProfessores) {
		this.listaProfessores = listaProfessores;
	}
}
